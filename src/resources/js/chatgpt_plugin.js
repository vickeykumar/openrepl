    ...
    return editor.windowManager.open({
      ...
      body: {
        ...
        items: [
          ...
          },
          {
            type: 'input',
            name: 'api_key',
            label: 'Enter Access Token or chatgpt Api Key',
            value: OPENAI.api_key
          },
          {
            type: 'htmlpanel',
            html: '<p style="font-size: 11px;text-align: center;margin-top:12px;">An opensource plugin for <a href="https://openai.com/" target="_blank">ChatGPT</a>.</p>'
          }
        ]
      },
      buttons: [
      ...
      onSubmit: function (api) {
        ...
        getResponseFromOpenAI(prompt)
          ...
          .then((data) => {
            // Define the reply
            const reply = `<pre>${data.choices[0].message.content}</pre>`;
            ...
            tinymce.activeEditor.windowManager.open({
              ...
              onSubmit: function (confirmapi) {
                    editor.insertContent(replyWithLineBreaks);
                    confirmapi.close();
                    api.close();
              }
            });
          ...
          .catch((error) => {
            ...
            tinymce.activeEditor.windowManager.alert('An error occurred: '+error);
          })
          .finally(() => {
            // Unblock the UI
            api.unblock();
